...
function DayColumn({ id, dayName, dayNumber, isToday, children }: DayColumnProps) {
  ...
  return (
    <div 
      ref={setNodeRef}
      className={`flex-1 min-w-[130px] flex flex-col h-full rounded-[35px] p-3 border transition-all ${
        isToday 
    ...
    >
      <div className="mb-4 px-2 shrink-0">
        <span className={`text-[9px] font-black uppercase tracking-[0.1em] ${isToday ? 'text-[#cff178]' : 'text-white/30'}`}>
          {dayName}
        </span>
        <h3 className="text-xl font-black text-white">{dayNumber}</h3>
      </div>
...
function SortableTaskCard({ task, colorClass, onToggle }: SortableTaskCardProps) {
  ...
  return (
    <div 
      ...
      className={`p-3 rounded-[22px] shadow-sm cursor-grab active:cursor-grabbing transition-all ${
        task.completed ? 'opacity-40 bg-white/10' : colorClass
      }`}
    >
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <span className="text-[9px] font-black flex items-center gap-1 opacity-70">
            <Clock size={10} /> {task.time}
          </span>
          <button onPointerDown={(e) => e.stopPropagation()} onClick={() => onToggle(task.id)}>
            {task.completed ? <CheckCircle2 size={14} /> : <Circle size={14} className="opacity-40" />}
          </button>
        </div>
        <h4 className={`text-[12px] font-bold leading-tight ${task.completed ? 'line-through opacity-50' : ''}`}>
          {task.title}
        </h4>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="h-[calc(100vh-160px)] flex flex-col p-4 animate-in fade-in duration-700">
        ...
        <div className="flex-1 flex gap-3 min-h-0 overflow-x-hidden">
          {days.map((day) => {
