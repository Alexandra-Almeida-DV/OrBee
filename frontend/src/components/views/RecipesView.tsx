  ...
  return (
    <div className="p-8 pb-24 animate-in fade-in duration-700">
      ...
      <div className="max-w-4xl mx-auto mb-16">
        <div className="bg-white rounded-[40px] shadow-2xl border border-white overflow-hidden relative">
          <div className="h-4 bg-[#FFB085] w-full" />
          <div className="p-10 pt-8">
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-1/3">
                ...
                {image ? (
                  ...
                  <button onClick={() => fileInputRef.current?.click()} className="w-full h-64 border-4 border-dashed border-orange-50 rounded-3xl flex flex-col items-center justify-center text-[#FFB085] hover:bg-orange-50 transition-colors gap-3">
                    <ImageIcon size={40} strokeWidth={1} />
                    <span className="text-xs font-black uppercase">Adicionar Foto</span>
                  </button>
                )}
              </div>

              <div className="flex-1 space-y-6">
                <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Nome da Receita..." className="w-full bg-transparent border-b-2 border-orange-100 text-3xl font-black text-[#5D5A88] outline-none pb-2 focus:border-[#FFB085] transition-all" />
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-orange-50/50 p-4 rounded-3xl border border-orange-100">
                    <div className="flex items-center gap-2 mb-2 text-[#FFB085]">
                      <Clock size={16} />
                      <span className="text-[10px] font-black uppercase">Preparo</span>
                    </div>
                    <input value={prepTime} onChange={(e) => setPrepTime(e.target.value)} placeholder="Ex: 20 min" className="bg-transparent w-full text-[#5D5A88] font-bold outline-none" />
                  </div>
                  <div className="bg-orange-50/50 p-4 rounded-3xl border border-orange-100">
                    <div className="flex items-center gap-2 mb-2 text-[#FFB085]">
                      <Flame size={16} />
                      <span className="text-[10px] font-black uppercase">Forno</span>
                    </div>
                    <input value={ovenTime} onChange={(e) => setOvenTime(e.target.value)} placeholder="Ex: 40 min" className="bg-transparent w-full text-[#5D5A88] font-bold outline-none" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 grid md:grid-cols-2 gap-8">
              <div className="relative bg-[#FDFDFD] p-6 rounded-3xl border border-gray-100">
                <div className="absolute left-8 top-0 bottom-0 w-[1px] bg-red-100" />
                <h3 className="text-[10px] font-black text-[#8A88B6] uppercase mb-4 pl-6 flex items-center gap-2"><BookOpen size={14} /> Ingredientes</h3>
                <textarea value={ingredients} onChange={(e) => setIngredients(e.target.value)} style={paperLines} className="w-full bg-transparent border-none text-[#5D5A88] min-h-[150px] outline-none pl-6 resize-none" />
              </div>
              <div className="relative bg-[#FDFDFD] p-6 rounded-3xl border border-gray-100">
                <div className="absolute left-8 top-0 bottom-0 w-[1px] bg-red-100" />
                <h3 className="text-[10px] font-black text-[#8A88B6] uppercase mb-4 pl-6">👩‍🍳 Modo de Preparo</h3>
                <textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} style={paperLines} className="w-full bg-transparent border-none text-[#5D5A88] min-h-[150px] outline-none pl-6 resize-none" />
              </div>
            </div>

            <div className="mt-8 flex justify-end">
              <button onClick={handleSave} className="flex items-center gap-3 bg-[#FFB085] hover:bg-[#ff9d66] text-white px-10 py-4 rounded-2xl font-black transition-all shadow-lg active:scale-95">
